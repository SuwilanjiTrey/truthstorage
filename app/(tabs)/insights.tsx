...
const CARDS = [
  {
    ...
    title: '128 GB ≠ 128 GB',
    summary: "Your device was sold as 128 GB but actual writable space is less.",
  ...
  },
  {
    ...
    detail: "Apps create cache to speed up repeated operations — loading images, storing session data. Android automatically evicts cache when storage runs low. Manually clearing it forces apps to re-download everything, which temporarily frees space but the data comes right back.",
  },
  {
  ...
  },
  {
    ...
    detail: "/data is the internal app storage partition — encrypted and protected. /sdcard (internal shared storage) is where photos, downloads, and media go. They're separate regions on the same flash chip. Confusing them leads to wildly wrong storage reports.",
  },
  {
...
export default function InsightsScreen() {
  ...
  const diff       = reportedGB - actualGB;
  ...
  return (
    <SafeAreaView style={s.safe}>
      <ScrollView contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
        ...
        {/* Live mismatch callout - only show if we have real data */}
        {storage.totalBytes > 0 && (
          <View style={s.callout}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 6 }}>
              <View style={s.calloutDot} />
              <Text style={s.calloutTag}>
                {diff > 2 ? 'MISMATCH DETECTED' : 'STORAGE VERIFIED'}
              </Text>
            </View>
            <Text style={s.calloutTitle}>
              Reported {reportedGB} GB → Actual {actualGB} GB
            </Text>
            <Text style={s.calloutSub}>
              {diff > 2
                ? `${diff} GB unaccounted for by system overhead and OEM measurement practices.`
                : 'Reported and actual storage are consistent.'}
            </Text>
          </View>
        )}
